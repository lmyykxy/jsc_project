...
`else
    always @(dqs or ~dqs) dqs_in <= #BUS_DELAY (~dqs<<16) | dqs;
...
`endif
    ...
    begin
        if (write_levelization && i<16) begin
            ...
            //if ($time - tm_dqs_pos[i] < $rtoi(TDQSH*tck_avg)) //edit by jiangde
            if ($time - tm_dqs_pos[i] < $rtoi(TDQSH*tck_avg) && dqs != {DQS_BITS{1'bz}})
